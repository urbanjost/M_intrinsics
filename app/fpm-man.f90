...
namelist/fman_colors/bg,fg,prg,head,head_,fixed,output,output_
   ...
   query=sget('query')
   write(*,*)'GOT HERE A:',get_env('LINES','0'),sget('lines')
   lines=iget('lines')
   write(*,*)'GOT HERE B:LINES:',lines
   if(.not.system_isatty(stdout))lines=0
   write(*,*)'GOT HERE C:LINES:',lines
                  ...
                  case('l')
                             ...
                             if(paws(2:).eq.'')then
                                ...
                                if(iostat.eq.0)then
                                   ...
                                   m=nint(rm)
                                   if(m.lt.0)then
                                      m=get_env('LINES',lines) ! adjust for screen size change if set
                                   endif
                                   lines=max(m,0)
                                endif
                             endif
